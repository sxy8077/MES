...
axios.interceptors.request.use(function(config){
  // 从 cookie 中获取token
  // config.headers.Authorization = window.sessionStorage.getItem('token')
	let token = getCookie('user');//必须采用解码，不然token.state返回的是undefiend
	console.log('token:',token);
	if(token){
		console.log('有数据')
		console.log('token:',token['token'])
		config.headers.token = JSON.parse(getCookie('user')).token;
		config.headers.user_id = JSON.parse(getCookie('user')).id;
	}
	// config.headers.token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJuYW1lIjoieGpzIiwiaWQiOiIxMzIwMjQwMTA5MTM3MjQwMDY1IiwiZXhwIjoxNjA0ODI2MTgyfQ.2rheWQtfDhjnp3flMca3B12VHCb2hC3SNgb6NjpDLqw'
 //  config.headers.user_id = 'f663c62b68354a58b7ccc210628d328f';
	// 必须 return config
  return config
},function(error){
